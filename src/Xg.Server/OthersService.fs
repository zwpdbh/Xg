...
type OthersService(ctx: IRemoteContext, env: IWebHostEnvironment) =
  inherit RemoteHandler<Client.Others.OthersService>()
  ...
  let books =
    ...
    JsonSerializer.Deserialize<Client.Others.Book []>(json)
    |> ResizeArray

  override this.Handler =
    { getBooks =
        ctx.Authorize
        <| fun () -> async { return books.ToArray() }

      addBook =
        ctx.Authorize
        <| fun book -> async { books.Add(book) }
